...
async function callAnthropic(prompt: string, apiKey: string): Promise<{ text: string; model: string }> {
    ...
    async function callModel(model: string): Promise<string> {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                console.error("[deep-analysis] Anthropic error:", res.status, errData);
    ...
    try {
        ...
        if (e.type === "not_found") {
            console.warn("[deep-analysis] Primary model not found, trying fallback:", FALLBACK_MODEL);
