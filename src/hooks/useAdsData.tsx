...
export function useAdsData(clientId?: string) {
  ...
  const fetchData = useCallback(async (range: DateRangeOption = dateRange) => {
    ...
    try {
      ...
      const campaignMap = new Map<string, { name: string; status: string; spend: number; clicks: number; conversions: number; leads: number; purchases: number; registrations: number; messages: number; followers: number; profile_visits: number; revenue: number; cpa: number; source: string; adset_count: number; ad_count: number }>();
      for (const row of (campaignRows || []) as Array<{ campaign_name: string; campaign_status: string; spend: number; clicks: number; conversions: number; leads: number; purchases?: number; registrations?: number; messages: number; followers?: number; profile_visits?: number; revenue: number; source: string; external_campaign_id?: string; adset_count?: number; ad_count?: number }>) {
        ...
        if (existing) {
          ...
          // Keep max adset_count and ad_count (same campaign across days)
